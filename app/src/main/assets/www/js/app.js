...
const MODES = {car:'🚗 Auto',camper:'🚐 Camper',walk:'🚶 Zu Fuß',transit:'🚌 ÖPNV',ferry:'⛴ Fähre',train:'🚆 Zug',flight:'✈ Flug'};
...
function emptyState(){return {schema:1,activeTripId:null,settings:defaultSettings(),trips:[],cache:{places:{},wikidata:{},wildlife:{},photos:{}}};}
...
function sampleTrip(){
  ...
  const v={id:uid('ver'),name:'Nordroute – Ausgangsplanung',startDate:start,endDate:addDays(start,13),days:[],flights:[],createdAt:Date.now()};
...
function loadState(){
  ...
  state.settings={...defaultSettings(),...(state.settings||{})}; state.cache=state.cache||{places:{},wikidata:{},wildlife:{},photos:{}};
}
...
function cloneVersion(v,newName){const c=JSON.parse(JSON.stringify(v));c.id=uid('ver');c.name=newName||`${v.name} – Kopie`;c.createdAt=Date.now();c.days.forEach(d=>{d.id=uid('day');d.stops.forEach(s=>s.id=uid('stop'));});c.flights.forEach(f=>f.id=uid('flight'));return c}
...
const Providers={
  ...
  async cityPois(lat,lng,radius=4500){
    ...
    const uniq=new Map();arr.sort((a,b)=>b.score-a.score).forEach(x=>{if(!uniq.has(normalizeName(x.name)))uniq.set(normalizeName(x.name),x)});return [...uniq.values()].slice(0,20);
  },
...
function dayConstraints(v,day){
  ...
  for(const f of v.flights||[]){if(f.arriveDate===day.date){const x=timeToMin(f.arriveTime);if(x!=null){const z=x+(+f.arrivalBuffer||state.settings.arrivalBuffer);if(z>start){start=z;notes.push(`nach Ankunft ${esc(f.number||f.to)} ab ${minToTime(z)}`)}}}if(f.departDate===day.date){const x=timeToMin(f.departTime);if(x!=null){const z=x-(+f.departureBuffer||state.settings.departureBuffer);if(z<end){end=z;notes.push(`vor Abflug ${esc(f.number||f.from)} bis ${minToTime(z)}`)}}}}
...
function renderPlanner(){
  ...
  box.innerHTML='';v.days.forEach((day,di)=>{const comp=computeDay(v,day),el=document.createElement('div');el.className='daycard';const flights=(v.flights||[]).filter(f=>f.departDate===day.date||f.arriveDate===day.date);el.innerHTML=`
    ...
    <div class="daybody" id="body_${day.id}"><div class="daytools"><button data-day-time="${day.id}">⏱ Tageszeit</button><button data-day-add="${day.id}">+ Stopp</button><button data-city-tour="${day.id}">🏙 Stadttour</button><button data-route-day="${day.id}">↻ Strecken</button></div>
...
function flightHtml(f){return `<div class="stop flightcard"><div class="stoprow"><div class="stopicon">✈</div><div><h4>${esc(f.number||'Flug')} · ${esc(f.from)} → ${esc(f.to)}</h4><div class="stopmeta">${fmtShortDate(f.departDate)} ${esc(f.departTime)} → ${fmtShortDate(f.arriveDate)} ${esc(f.arriveTime)}${f.notes?`<br>${esc(f.notes)}`:''}</div></div><button data-edit-flight="${f.id}">✎</button></div></div>`}
function stopHtml(day,r,i,count,di){const s=r.stop,leg=s.routeToNext&&i<count-1?s.routeToNext:null;return `<div class="stop" data-stop="${s.id}"><div class="stoprow"><div class="stopicon">${STOP_ICONS[s.type]||'•'}</div><div><h4>${minToTime(r.arrival)}–${minToTime(r.departure)} · ${esc(s.name)}</h4><div class="stopmeta"><span class="priority ${s.priority}">${priorityText(s.priority)}</span>${durText(s.durationWish||s.durationMin)}${s.openingHours?` · geöffnet: ${esc(s.openingHours)}`:''}${s.notes?`<br>${esc(s.notes)}`:''}</div></div><div class="stopactions"><button data-move-stop="${s.id}" data-dir="-1">↑</button><button data-move-stop="${s.id}" data-dir="1">↓</button><button data-edit-stop="${s.id}">✎</button><button data-delete-stop="${s.id}">×</button></div></div></div>${leg?`<div class="leg"><div class="legline"></div><div>${MODES[s.modeToNext]||s.modeToNext} · ${kmText(leg.distanceKm)} · ${durText(leg.durationMin)}${leg.approx?' · ca.':''}</div></div>`:''}`}
...
function fitMap(){if(!mapReady)return;const v=activeVersion(),pts=v?.days.flatMap(d=>d.stops).filter(s=>Number.isFinite(s.lat)&&Number.isFinite(s.lng)).map(s=>[s.lat,s.lng])||[];if(pts.length)map.fitBounds(pts,{padding:[25,25]})}
...
function bindEvents(){
  ...
  $('#newTripBtn').onclick=newTripModal; $('#refreshBtn').onclick=refreshAllRoutes; $('#discoverBtn').onclick=discoverAll;$('#reloadPoiBtn').onclick=loadAttractions;$('#reloadWildBtn').onclick=loadWildlife;$('#addStopBtn').onclick=()=>addStopModal();$('#addFlightBtn').onclick=()=>flightModal();$('#fitMapBtn').onclick=fitMap;$('#mapFilterBtn').onclick=mapFilterModal;
  ...
  document.addEventListener('click',e=>{
    ...
    if(d.editStop){const x=locateStop(d.editStop);if(x)editStopModal(x.day,x.stop,false)}
